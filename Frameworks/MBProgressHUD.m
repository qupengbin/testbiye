...
+ (MBProgressHUD *)showHUDAddedTo:(UIView *)view animated:(BOOL)animated reservedNav:(BOOL)reservedNav {
	MBProgressHUD *hud = [[MBProgressHUD alloc] initWithView:view reservedNav:reservedNav];
	[view addSubview:hud];
	[hud show:animated];
...
- (id)initWithView:(UIView *)view reservedNav:(BOOL)areservedNav {
	NSAssert(view, @"View must not be nil.");
    self.reservedNav = areservedNav;
    CGRect r = view.bounds;
    if (self.reservedNav) {
        r.origin.x = 0;
        r.origin.y = 44;
        r.size.height  = r.size.height - 44;
        
    }
	id me = [self initWithFrame:r];
...
- (id)initWithWindow:(UIWindow *)window {
	return [self initWithView:window reservedNav:NO];
}
