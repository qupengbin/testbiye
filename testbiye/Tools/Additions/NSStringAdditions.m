...
- (id)urlDecoded {
    ...
    NSString *decodedString = [NSString stringWithString:(NSString *)decodedCFString];
...
- (BOOL) validateUsername{
    NSString *unameRegex = @"([a-z0-9A-Z._]){6,20}";
    NSPredicate *preUname = [NSPredicate predicateWithFormat:@"SELF MATCHES %@",unameRegex];
    return [preUname evaluateWithObject:self];
}

- (BOOL) validatePassword{
    NSString *pwdRegex = @"([\\S]){6,20}"; // 匹配6-20位任意非空白字符
    NSPredicate *prePwd = [NSPredicate predicateWithFormat:@"SELF MATCHES %@",pwdRegex];
    return [prePwd evaluateWithObject:self];
}

- (BOOL) valideNumber {
    
    NSString *numRegex = @"([1-9]+\\.[0-9]+|0\\.[0-9]+|[1-9]+\\d*)";
    NSPredicate *numPwd = [NSPredicate predicateWithFormat:@"SELF MATCHES %@",numRegex];
    return [numPwd evaluateWithObject:self];
}
